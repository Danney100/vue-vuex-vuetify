...
export const menuitems = {
  ...
  actions: {
        ...
        if (this.state.authentication.role) {
          if (guardAPI('vendors/societies', this.state.authentication.role.id)) {
            axios.get('vendors/societies')
              .then(res => {
                commit({ type: 'SET_SOCIETY_ITEMS', data: res.data.data })
                resolve()
              })
              .catch(err => reject(err))
          } else resolve()
    ...
    getCompanyInfo ({ commit }) {
      return new Promise((resolve, reject) => {
        if (this.state.authentication.role) {
          if (guardAPI('company/getCompanyInfo', this.state.authentication.role.id)) {
            axios.get('company/getCompanyInfo')
              .then(res => {
                commit({ type: 'SET_COMPANY_INFO', data: res.data })
                resolve()
              })
              .catch(err => reject(err))
          } else resolve()
          ...
          this.watch(state => state.authentication.role, role => {
            if (role && guardAPI('company/getCompanyInfo', role.id)) {
              axios.get('company/getCompanyInfo')
                .then(res => {
                  commit({ type: 'SET_COMPANY_INFO', data: res.data })
                  resolve()
