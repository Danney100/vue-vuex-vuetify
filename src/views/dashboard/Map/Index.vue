          ...
          >
            ...
            >
              ...
              <v-list-item
                ...
                :key="fleet.id"
                class="pl-10"
                @click="onFleetsChanged(fleet.id)"
              >
                <v-checkbox
            ...
            >
              ...
              <v-list-item
                ...
                :key="network.id"
                class="pl-10"
                @click="onNetworksChanged(network.id)"
              >
                <v-checkbox
...
<script>
  ...
  import { tileProviders, toggleSelection } from './helpers'
  ...
  export default {
    ...
    data: () => ({
      ...
      timestampFilter: TIMESTAMP_FILTERS[2].value,
      // easyPrinter: null,
      infoDrawerOptions: {
        id: 0,
        type: '',
    ...
    methods: {
      ...
      /**
       * fleet is only related to vessels
       */
      async onFleetsChanged (fleetId) {
        this.selectedFleets = toggleSelection(this.selectedFleets, fleetId)
        this.getAndRenderVessels()
        const fleet = this.fleets.find(fleet => fleet.id === fleetId)
        fleet.checked = !fleet.checked
      },

      async onNetworksChanged (networkId) {
        this.selectedNetworks = toggleSelection(this.selectedNetworks, networkId)
        this.getAndRenderVessels()
        this.getAndRenderIndividuals()
        this.getAndRenderCompanies()
      },
      ...
      exportKML () {
        // extract ids for selectedZones
        // const zones = this.selectedZones.map(zone => zone.id)
        // if (zones.includes('us_maritime_boundaries_and_limits')) {
        //   zones.splice(zones.indexOf('us_maritime_boundaries_and_limits'), 1)
        //   if (!zones.includes('us_maritime_limits_and_boundaries')) {
        //     zones.push('us_maritime_limits_and_boundaries')
        //   }
        // }
        // if (this.sections[0].zones[1].checked) zones.push('piers')
        ...
        axios.get('map/export/CDT.kml', { responseType: 'arraybuffer' })
          .then(res => {
