            ...
            >
              ...
              >
                ...
                <v-btn
                  ...
                  x-small
                  color="success"
                  :href="'https://storage.googleapis.com/donjon-smit/files/' + directory.code + '/' + companyID + '/' + directory.id + '/' + directory.year + '/' + item.name"
                  target="_blank"
                >
                ...
                <v-btn
                  ...
                  x-small
                  color="success"
                  :href="'https://storage.googleapis.com/donjon-smit/files/' + directory.code + '/' + companyID + '/' + directory.id + '/' + item.name"
                  target="_blank"
                >
                  <v-icon>mdi-eye-check</v-icon>
                </v-btn>
    ...
    data () {
      return {
        ...
        dropOptions: {
          ...
          dictDefaultMessage: '<b>Drag and drop to upload content</b><br/> or click to select a file from your computer.<br/>Maximum 5 files upload at a time. Maximum per file upload size 256mb.',
          timeout: 18000000,
          parallelUploads: 5,
        },
        fab: {},
      ...
      async getDataFromApi () {
        ...
        const intervalId = setInterval(() => {
          if (this.companyItems.length) {
            ...
            this.companyName = result.company_id ? this.companyItems.filter(company => company.id === result.company_id)[0].name : 'No company assigned'
            this.loading = false
          }
        }, 50)
        this.loading = false
